...
// Shared polished input class — focus glow applied via globals.css
const inputClass = [
...
export function AddBookModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal — uses animate-scale-in (scale + translateY) */}
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl animate-scale-in">
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-semibold text-foreground">Add New Book</h2>
          </div>
          <button
            onClick={onClose}
            className={[
